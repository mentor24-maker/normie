...
export async function PATCH(
  ...
  try {
    ...
    const updates: {
      name: string;
      module_class: string;
      modules?: ReturnType<typeof normalizeBuilderModules>;
      updated_at: string;
    } = {
      name,
    ...
    if (error || !data) {
      return auth.finish(
        NextResponse.json({ error: error?.message ?? "Failed to update saved module." }, { status: 500 })
      );
    }

    return auth.finish(NextResponse.json({ cellModule: rowToBuilderCellModule(data) }));
  } catch (error) {
    console.error("PATCH /api/admin/cell-modules/[id] failed:", error);

    return auth.finish(
      NextResponse.json(
        {
          error: error instanceof Error ? error.message : "Failed to update saved module."
        },
        { status: 500 }
      )
    );
...
export async function DELETE(
  ...
  try {
    ...
    console.error("DELETE /api/admin/cell-modules/[id] failed:", error);

    return auth.finish(
      NextResponse.json(
        {
          error: error instanceof Error ? error.message : "Failed to delete saved module."
        },
        { status: 500 }
      )
    );
