...
export async function POST(request: Request) {
  ...
  if (error || !data) {
    return auth.finish(
      NextResponse.json(
        {
          error: error?.message.includes("game_level_tiers")
            ? "Missing game_level_tiers table. Apply migration 020_game_management.sql."
            : error?.message ?? "Failed to save level tier."
        },
        { status: 500 }
      )
    );
  ...
  return auth.finish(NextResponse.json({ levelTier: gameLevelTierToClient(data) }, { status: 201 }));
}
