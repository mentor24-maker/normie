...
export async function POST(request: Request) {
  ...
  if (error || !data) {
    return auth.finish(
      NextResponse.json(
        {
          error: error?.message.includes("game_interstitials")
            ? "Missing game_interstitials table. Apply migration 054_game_interstitials.sql."
            : error?.message ?? "Failed to save interstitial."
        },
        { status: 500 }
      )
    );
  ...
  return auth.finish(NextResponse.json({ interstitial: gameInterstitialToClient(data) }, { status: 201 }));
}
